"""
=============================================================
  Lung Cancer Prediction -- Classification Metrics
=============================================================
  Dataset : Survey Lung Cancer (Kaggle)
  Model   : Logistic Regression
  Metrics : Accuracy, Precision, Recall, F1 Score
  
  Kaggle Link: https://www.kaggle.com/datasets/mysarahmadbhat/lung-cancer
=============================================================
"""
...
OUTPUT_DIR = os.path.join(SCRIPT_DIR, "output_charts_lung")
os.makedirs(OUTPUT_DIR, exist_ok=True)

# ─────────────────────────────────────────────────────────────
# STEP 2: Load the Lung Cancer Dataset
# ─────────────────────────────────────────────────────────────
csv_path = os.path.join(SCRIPT_DIR, "lung_cancer_data.csv")
df = pd.read_csv(csv_path)

print("=" * 60)
print("  📊 LUNG CANCER DATASET — Overview")
print("=" * 60)
print(f"\n  Shape       : {df.shape[0]} rows × {df.shape[1]} columns")
print(f"  Features    : {df.shape[1] - 1}")
print(f"  Target      : 'LUNG_CANCER' (YES = Cancer, NO = No Cancer)")
print()
...
print(f"  YES (Lung Cancer)    : {target_counts.get('YES', 0)} patients")
print(f"  NO  (No Lung Cancer) : {target_counts.get('NO', 0)} patients")
print()
...
features_info = {
    ...
    'SMOKING': 'Smoking habit (2=Yes, 1=No)',
    'YELLOW_FINGERS': 'Yellow fingers (2=Yes, 1=No)',
    'ANXIETY': 'Anxiety (2=Yes, 1=No)',
    'PEER_PRESSURE': 'Peer pressure (2=Yes, 1=No)',
    'CHRONIC_DISEASE': 'Chronic disease (2=Yes, 1=No)',
    'FATIGUE': 'Fatigue (2=Yes, 1=No)',
    'ALLERGY': 'Allergy (2=Yes, 1=No)',
    'WHEEZING': 'Wheezing (2=Yes, 1=No)',
    'ALCOHOL_CONSUMING': 'Alcohol consuming (2=Yes, 1=No)',
    'COUGHING': 'Coughing (2=Yes, 1=No)',
    'SHORTNESS_OF_BREATH': 'Shortness of breath (2=Yes, 1=No)',
    'SWALLOWING_DIFFICULTY': 'Swallowing difficulty (2=Yes, 1=No)',
    'CHEST_PAIN': 'Chest pain (2=Yes, 1=No)',
    'LUNG_CANCER': 'Target: YES = Lung Cancer, NO = No Lung Cancer'
}
...
# Convert the binary features from (1, 2) → (0, 1) for consistency
...
print("  Binary cols:  2 → 1 (Yes), 1 → 0 (No)")
print()

# ─────────────────────────────────────────────────────────────
# STEP 6: Visualize the Data
# ─────────────────────────────────────────────────────────────
# Chart 1: Class Distribution + Age Distribution
fig, axes = plt.subplots(1, 2, figsize=(14, 5))

colors_target = ['#FF6B6B', '#4ECDC4']
labels = ['No Cancer (0)', 'Lung Cancer (1)']
target_counts_sorted = df['LUNG_CANCER'].value_counts().sort_index()
...
for i, label in enumerate([0, 1]):
    subset = df[df['LUNG_CANCER'] == label]
...
# ─────────────────────────────────────────────────────────────
# STEP 7: Prepare the Data (Train-Test Split)
# ─────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────
# STEP 9: Calculate ALL 4 Classification Metrics
# ─────────────────────────────────────────────────────────────
...
# ─────────────────────────────────────────────────────────────
# STEP 10: What Do These Metrics Mean? (Formulas)
# ─────────────────────────────────────────────────────────────
print()
print("─" * 60)
...
print("""
  ...
  ├────────────┼─────────────────────────────────────────────────────────────┤
  │ Precision  │ TP / (TP + FP)                                             │
  │            │ → When model says "Cancer", how often is it correct?        │
  ├────────────┼─────────────────────────────────────────────────────────────┤
  │ Recall     │ TP / (TP + FN)                                             │
  │            │ → Of all actual Cancer patients, how many did we find?      │
  ├────────────┼─────────────────────────────────────────────────────────────┤
...
print(classification_report(
    y_test, y_pred,
    target_names=['No Cancer (0)', 'Lung Cancer (1)']
))

# ─────────────────────────────────────────────────────────────
# STEP 12: Confusion Matrix (Chart)
# ─────────────────────────────────────────────────────────────
...
disp = ConfusionMatrixDisplay(
    confusion_matrix=cm,
    display_labels=['No Cancer', 'Lung Cancer']
)
...
# ─────────────────────────────────────────────────────────────
# STEP 13: Bar Chart — Compare All 4 Metrics
# ─────────────────────────────────────────────────────────────
...
print()
print("=" * 70)
print("  📊 FINAL SUMMARY — Lung Cancer Classification Metrics")
print("  Dataset: Survey Lung Cancer  |  Model: Logistic Regression")
...
summary_df = pd.DataFrame({
    ...
    'Meaning': [
        'Overall correctness of the model',
        'When predicting cancer, how often correct?',
